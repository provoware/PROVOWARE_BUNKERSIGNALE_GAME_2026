...
def main() -> int:
    ...
    try:
        ...
        if issues:
            ...
            checks.append({"name": "repository_validation", "status": "PASS", "detail": "Pflichtdateien, Versionen, Standards, Change-Historie, Registry und Architekturfixtures sind gültig."})
        ...
        current_checkpoint = str(read_json(ROOT / "manifests/project.manifest.json").get("checkpoint", "I00"))
        if current_checkpoint == "I00":
            write_evidence(status, fingerprint, hashes, checks, unit_output)

        print(f"REPOSITORY QUALITY STATUS: {status}")
        print(f"Fingerprint: {fingerprint}")
        if current_checkpoint == "I00":
            print("Evidence: evidence/I00_EVIDENCE.txt")
        else:
            print(f"Frozen I00 evidence: unchanged (current checkpoint {current_checkpoint})")
