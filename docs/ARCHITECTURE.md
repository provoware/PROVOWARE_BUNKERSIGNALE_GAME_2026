...
`manifests/content.registry.json` definiert ausschließlich exakt versionierte, lokale Contentpakete und deren Abhängigkeiten. `manifests/content.lock.json` pinnt Pfad und SHA-256 jedes registrierten Pakets. `tools/content_registry.py` löst Abhängigkeiten deterministisch auf, verbietet Repository-Escape-Pfade und prüft Datei-Hashes gegen das Lockfile. Registry und Lockfile besitzen zusätzlich einen gemeinsamen Drift-Fingerprint in `manifests/content.registry-lock.sha256`. Seit I04 prüft `tools/content_inbox.py` einen lokalen Kandidaten vor jeder Verschiebung gegen Identität und Lock-Pin: valide Kandidaten werden atomar an ihr registriertes Ziel verschoben, invalide Kandidaten ohne Überschreiben eines vorhandenen Ziels quarantänisiert. Registry und Lockfile bleiben dabei unverändert.
