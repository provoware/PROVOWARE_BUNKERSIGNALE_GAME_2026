...
**P2-LI – Sign-Orchestration Integration Gate:** Nach grün implementiertem P2-L ist vor P3 noch ein eigener Integrationsschritt erforderlich. Grund: `ensureActiveSigningKeyRegistered(...)` und `signEventDetached(...)` existieren derzeit als getrennt aufrufbare Application-Capabilities; damit ist die Registration Barrier noch nicht durch einen einzigen freigegebenen Sign-Pfad erzwungen. Vor P3 MUSS daher ein minimaler Application-Orchestrator eingefroren werden, der `ensureActiveSigningKeyRegistered(...)` erfolgreich abschließt, ausschließlich dessen bestätigten `key_id` verwendet und erst danach `signEventDetached(...)` aufruft. Direkte Signaturpersistenz bleibt dabei weiterhin verboten. P3 darf erst nach grünem P2-LI-Freeze eröffnet werden.

**P2-LI Contract Gate – Registered Sign Path:** Der minimale öffentliche Orchestrator wird als `signEventWithRegisteredActiveKey(context, capabilities)` eingefroren. `context` besitzt exakt `worldId`, `event`, `chainEntry`, `i11Verified`; ein externer `keyId` oder zusätzliche Felder sind fail-closed verboten. Der Orchestrator MUSS zuerst P2-L erfolgreich abschließen, ausschließlich dessen bestätigten `key_id` in den internen I12-A-Signaturkontext übernehmen und erst danach `signEventDetached(...)` ausführen. P2-L-Fehler bleiben unverändert; P2-LI ergänzt nur stabile Codes für ungültigen Context, ungültige Capability-Verdrahtung und Signaturfehler nach erfolgreicher Registration. Keine Signature-Persistence-Capability ist erlaubt. P3 bleibt bis zum grünen P2-LI-Implementierungs-Freeze gesperrt.

**P2-LV – Generated Signature Verification Barrier:** Nach grün implementiertem P2-LI ist P3 noch nicht unmittelbar freigegeben. Die injizierte `signBytes`-Capability kann durch Fehlverdrahtung theoretisch mit einem anderen privaten Schlüssel signieren als dem über P2-L registrierten `key_id`. Deshalb MUSS jeder lokal erzeugte detached Signature Record vor einer späteren Persistenz gegen den ausschließlich über P2 aufgelösten historischen Public Key und die bestehenden I12-A/B-Verifikationsregeln selbst geprüft werden. Nur ein erfolgreich verifizierter, unveränderter Record darf an einen späteren P3-Pfad weitergereicht werden. Verify=false sowie Key-Lookup-/Import-/Crypto-Fehler blockieren fail-closed. P3 bleibt bis zum grünen P2-LV-Implementierungs-Freeze gesperrt.

**P2-LV Contract Gate – Verified Local Signature Evidence:** Detailvertrag: `docs/I12_C_P2_LV_VERIFIED_LOCAL_SIGNATURE_EVIDENCE_CONTRACT.md`. Eingefroren sind ausschließlich der P2-Key-Lookup, bestehende I12-A/B-Verifikation, ein prozesslokaler nicht fälschbarer Evidence-Proof ohne `verified:Boolean`, stabile Fehlercodes und das vollständige Persistence-Verbot. P3 bleibt bis zum grünen P2-LV-Implementierungs-Freeze gesperrt.
## Non-Goals
