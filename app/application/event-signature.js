...
export function frameEventSignatureInput({ worldId, previousHash, eventHash, eventBytes }) {
  ...
  return concat(
    SIGNATURE_DOMAIN,
    encoder.encode(worldId),
    LF,
    encoder.encode(previousHash),
    LF,
    encoder.encode(eventHash),
    LF,
    eventBytes,
  );
}
...
export function validateEventSignatureRecord(record, { worldId, event }) {
  ...
  if (record.algorithm !== "Ed25519" || !SIGNATURE_BASE64URL.test(record.signature)) return false;
  return true;
}
...
export function createEventSignatureRecord({ worldId, event, keyId, signature }) {
  ...
  if (!validateEventSignatureRecord(record, { worldId, event })) {
    throw new TypeError("signature record is invalid");
  }
  return record;
}
